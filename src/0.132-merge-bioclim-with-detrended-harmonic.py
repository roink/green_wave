#!/usr/bin/env python3
"""Combine detrended harmonic fits with resampled bioclim layers."""
...
DETREND_PATH = INTERMEDIATE_DIR / "detrended_ndvi_harmonic_fit_semiannual.h5"
OUTPUT_PATH = INTERMEDIATE_DIR / "detrended_ndvi_bioclim_combined.npz"

FALLBACK_GRID_SPEC = NdviGridSpec(row_start=0, row_end=3600, col_start=0, col_end=7200)
...
def main() -> None:
    if not DETREND_PATH.exists():
        raise FileNotFoundError(
            "Detrended harmonic fit file missing. Expected {}.".format(DETREND_PATH)
        )

    bioclim_files = list_bioclim_files(RAW_BIOCLIM_DIR)
    if not bioclim_files:
        raise FileNotFoundError(
            "No WorldClim bioclim files found. Expected GeoTIFFs in {}.".format(
                RAW_BIOCLIM_DIR
            )
        )
    print(f"Found {len(bioclim_files)} bioclim layers to resample.")

    with h5py.File(DETREND_PATH, "r") as h5f:
    ...
    print(f"Saved detrended combined dataset to {OUTPUT_PATH}.")
